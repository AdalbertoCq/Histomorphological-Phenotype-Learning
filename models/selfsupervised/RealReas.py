...
class RepresentationsPathology():
	...
	def model_inputs(self):
		...
		transf_real_images_3 = tf.placeholder(dtype=tf.float32, shape=(self.batch_size, self.image_width, self.image_height, self.image_channels), name='transf_real_images_3')
		transf_real_images_4 = tf.placeholder(dtype=tf.float32, shape=(self.batch_size, self.image_width, self.image_height, self.image_channels), name='transf_real_images_4')
		transf_real_images_5 = tf.placeholder(dtype=tf.float32, shape=(self.batch_size, self.image_width, self.image_height, self.image_channels), name='transf_real_images_5')
		transf_real_images_6 = tf.placeholder(dtype=tf.float32, shape=(self.batch_size, self.image_width, self.image_height, self.image_channels), name='transf_real_images_6')
		# Learning rates.
		learning_rate_e = tf.placeholder(dtype=tf.float32, name='learning_rate_e')

		return real_images_1, real_images_2, transf_real_images_1, transf_real_images_2, transf_real_images_3, transf_real_images_4, transf_real_images_5, transf_real_images_6, learning_rate_e
	...
	def encoder(self, images, is_train, reuse, init, name, label_input=None):
		if 'ContrastivePathology_Relational_0' in self.model_name:
		...
		elif 'ContrastivePathology_Relational_1' in self.model_name:
			conv_space, h, z = encoder_resnet_contrastive_1(images=images, z_dim=self.z_dim, h_dim=1024, layers=self.layers, spectral=self.spectral, activation=ReLU, reuse=reuse, init=init,
															 is_train=is_train, normalization=batch_norm, regularizer=None, attention=self.attention, name=name)
		elif 'ContrastivePathology_Relational_2' in self.model_name:
			conv_space, h, z = encoder_resnet_contrastive_2(images=images, z_dim=self.z_dim, h_dim=1024, layers=self.layers, spectral=self.spectral, activation=ReLU, reuse=reuse, init=init,
															 is_train=is_train, normalization=batch_norm, regularizer=None, attention=self.attention, name=name)
		elif 'ContrastivePathology_Relational_3' in self.model_name:
			conv_space, h, z = encoder_resnet_contrastive_3(images=images, z_dim=self.z_dim, h_dim=1024, layers=self.layers, spectral=self.spectral, activation=ReLU, reuse=reuse, init=init,
															 is_train=is_train, normalization=batch_norm, regularizer=None, attention=self.attention, name=name)
		elif 'ContrastivePathology_Relational_4' in self.model_name:
			conv_space, h, z = encoder_resnet_contrastive_4(images=images, z_dim=self.z_dim, h_dim=1024, layers=self.layers, spectral=self.spectral, activation=ReLU, reuse=reuse, init=init,
															 is_train=is_train, normalization=batch_norm, regularizer=None, attention=self.attention, name=name)
		elif 'ContrastivePathology_Relational_5' in self.model_name:
	...
	def aggregational_module(self, z_rep_t1, z_rep_t2, z_rep_t3, z_rep_t4, z_rep_t5, z_rep_t6):

		representations  = [z_rep_t1, z_rep_t2, z_rep_t3, z_rep_t4, z_rep_t5, z_rep_t6]

		contrastive_samples = list()
		labels              = list()

		shifts_counter = 1
		for i, rep_1 in enumerate(representations):
			for j, rep_2 in enumerate(representations):
				if i==j: continue
				rep_2_shifted = tf.roll(rep_2, shift=shifts_counter, axis=0)

				comb_rep_pos = tf.concat([rep_1, rep_2], axis=1)
				comb_rep_neg = tf.concat([rep_1, rep_2_shifted], axis=1)

				contrastive_samples.append(comb_rep_pos)
				contrastive_samples.append(comb_rep_neg)

				labels.append(tf.ones((self.batch_size, 1)))
				labels.append(tf.zeros((self.batch_size, 1)))

				shifts_counter+=1
				# Avoid identity
				if  shifts_counter>=self.batch_size:
					shifts_counter=1

		contrastive_samples = tf.concat(contrastive_samples, axis=0)
		labels              = tf.concat(labels,              axis=0)

		return contrastive_samples, labels

	def relational_reasoning_module(self, contrastive_samples, is_train, reuse, init, name='relational_module'):
		logits = relational_module(aggregated_representations=contrastive_samples, h_dim=256, spectral=self.spectral, activation=leakyReLU, is_train=is_train, reuse=reuse, init=init, regularizer=None, normalization=batch_norm, name=name)
		return logits

	# Loss Function.
	def loss(self, labels, logits):
		loss_contrastive = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=labels, logits=logits))
	...
	def build_model(self):
		...
		# Inputs.
		self.real_images_1, self.real_images_2, self.transf_real_images_1, self.transf_real_images_2, self.transf_real_images_3, self.transf_real_images_4, \
		self.transf_real_images_5, self.transf_real_images_6, self.learning_rate_input_e = self.model_inputs()
		# Data augmentation layer.
		...
		self.real_images_1_t2 = self.data_augmentation_layer(images=self.real_images_1, crop=self.crop, rotation=self.rotation, flip=self.flip, g_blur=self.g_blur, g_noise=self.g_noise,
															 color_distort=self.color_distort, sobel_filter=self.sobel_filter)
		self.real_images_1_t3 = self.data_augmentation_layer(images=self.real_images_1, crop=self.crop, rotation=self.rotation, flip=self.flip, g_blur=self.g_blur, g_noise=self.g_noise,
															 color_distort=self.color_distort, sobel_filter=self.sobel_filter)
		self.real_images_1_t4 = self.data_augmentation_layer(images=self.real_images_1, crop=self.crop, rotation=self.rotation, flip=self.flip, g_blur=self.g_blur, g_noise=self.g_noise,
															 color_distort=self.color_distort, sobel_filter=self.sobel_filter)
		self.real_images_1_t5 = self.data_augmentation_layer(images=self.real_images_1, crop=self.crop, rotation=self.rotation, flip=self.flip, g_blur=self.g_blur, g_noise=self.g_noise,
															 color_distort=self.color_distort, sobel_filter=self.sobel_filter)
		self.real_images_1_t6 = self.data_augmentation_layer(images=self.real_images_1, crop=self.crop, rotation=self.rotation, flip=self.flip, g_blur=self.g_blur, g_noise=self.g_noise,
															 color_distort=self.color_distort, sobel_filter=self.sobel_filter)
		...
		self.conv_space_t3, self.h_rep_t3, self.z_rep_t3 = self.encoder(images=self.transf_real_images_3, is_train=True, reuse=True,  init=self.init, name='contrastive_encoder')
		self.conv_space_t4, self.h_rep_t4, self.z_rep_t4 = self.encoder(images=self.transf_real_images_4, is_train=True, reuse=True,  init=self.init, name='contrastive_encoder')
		self.conv_space_t5, self.h_rep_t5, self.z_rep_t5 = self.encoder(images=self.transf_real_images_5, is_train=True, reuse=True,  init=self.init, name='contrastive_encoder')
		self.conv_space_t6, self.h_rep_t6, self.z_rep_t6 = self.encoder(images=self.transf_real_images_6, is_train=True, reuse=True,  init=self.init, name='contrastive_encoder')

		# Aggregate representations.
		self.contrastive_samples, self.labels  = self.aggregational_module(self.z_rep_t1, self.z_rep_t2, self.z_rep_t3, self.z_rep_t4, self.z_rep_t5, self.z_rep_t6)

		# Relational Reasoning module.
		self.logits = self.relational_reasoning_module(contrastive_samples=self.contrastive_samples, is_train=True, reuse=False, init=self.init, name='relational_module')
		...
		################### LOSS & OPTIMIZER ##############################################################################################################################
		# Losses.
		self.loss_contrastive = self.loss(labels=self.labels, logits=self.logits)
		# Optimizers.
	...
	def project_subsample(self, session, data, epoch, data_out_path, report, batch_size=50):
		...
		while ind<num_samples:
			...
			ind += batch_size
		try:
			report_progress_latent(epoch=epoch, w_samples=conv_storage, img_samples=img_storage, img_path=hdf5_path.split('/hdf5')[0], storage_name='conv_lat', metric='cosine')
			report_progress_latent(epoch=epoch, w_samples=h_storage,    img_samples=img_storage, img_path=hdf5_path.split('/hdf5')[0], storage_name='h_lat', metric='cosine')
			report_progress_latent(epoch=epoch, w_samples=z_storage,    img_samples=img_storage, img_path=hdf5_path.split('/hdf5')[0], storage_name='z_lat', metric='cosine')
		except:
	...
	def train(self, epochs, data_out_path, data, restore, print_epochs=10, n_images=25, checkpoint_every=None, report=False):
		...
		with tf.Session(config=config) as session:
			...
			output_layers_transformed = [self.real_images_1_t1, self.real_images_1_t2, self.real_images_1_t3, self.real_images_1_t4, self.real_images_1_t5, self.real_images_1_t6]
			...
			write_sprite_image(filename=os.path.join(data_out_path, 'images/transformed_3.png'), data=transformed_images[2], metadata=False)
			write_sprite_image(filename=os.path.join(data_out_path, 'images/transformed_4.png'), data=transformed_images[3], metadata=False)
			write_sprite_image(filename=os.path.join(data_out_path, 'images/transformed_5.png'), data=transformed_images[4], metadata=False)
			write_sprite_image(filename=os.path.join(data_out_path, 'images/transformed_6.png'), data=transformed_images[5], metadata=False)
			...
			for epoch in range(1, epochs+1):
				...
				for batch_images, batch_labels in data.training:
					...
					feed_dict = {self.real_images_1:batch_images, self.real_images_2:batch_images}
					transformed_images = session.run(output_layers_transformed, feed_dict=feed_dict, options=run_options)
					feed_dict = {self.transf_real_images_1:transformed_images[0], self.transf_real_images_2:transformed_images[1], self.transf_real_images_3:transformed_images[2], \
								 self.transf_real_images_4:transformed_images[3], self.transf_real_images_5:transformed_images[4], self.transf_real_images_6:transformed_images[5], \
