...
class RepresentationsPathology():
	...
	def encoder(self, images, is_train, reuse, init, name, label_input=None):
		if 'ContrastivePathology_SimCLR_0' in self.model_name or 'ContrastivePathology_SimCLR_250k' in self.model_name:
		...
		if 'ContrastivePathology_SimCLR_1' in self.model_name:
			conv_space, h, z = encoder_resnet_contrastive_1(images=images, z_dim=self.z_dim, h_dim=1024, layers=self.layers, spectral=self.spectral, activation=ReLU, reuse=reuse, init=init,
															 is_train=is_train, normalization=batch_norm, regularizer=None, attention=self.attention, name=name)
		elif 'ContrastivePathology_SimCLR_2' in self.model_name:
			conv_space, h, z = encoder_resnet_contrastive_2(images=images, z_dim=self.z_dim, h_dim=1024, layers=self.layers, spectral=self.spectral, activation=ReLU, reuse=reuse, init=init,
															 is_train=is_train, normalization=batch_norm, regularizer=None, attention=self.attention, name=name)
		elif 'ContrastivePathology_SimCLR_3' in self.model_name:
			conv_space, h, z = encoder_resnet_contrastive_3(images=images, z_dim=self.z_dim, h_dim=1024, layers=self.layers, spectral=self.spectral, activation=ReLU, reuse=reuse, init=init,
															 is_train=is_train, normalization=batch_norm, regularizer=None, attention=self.attention, name=name)
		elif 'ContrastivePathology_SimCLR_4' in self.model_name:
			conv_space, h, z = encoder_resnet_contrastive_4(images=images, z_dim=self.z_dim, h_dim=1024, layers=self.layers, spectral=self.spectral, activation=ReLU, reuse=reuse, init=init,
															 is_train=is_train, normalization=batch_norm, regularizer=None, attention=self.attention, name=name)
		elif 'ContrastivePathology_SimCLR_5' in self.model_name:
	...
	# Loss Function.
	def loss(self):
		loss_contrastive, logits_ab_contrastive, labels_contrastive = contrastive_loss(a=self.z_rep_t1, b=self.z_rep_t2, batch_size=self.batch_size, temperature=self.temperature, weights=1.0)
		return loss_contrastive, logits_ab_contrastive, labels_contrastive
	...
	def optimization(self):
		...
		# trainable_variables = tf.trainable_variables()
		# zero_ops, accum_ops, train_step = optimizer_contrastive_accumulated_gradients(loss=self.loss_contrastive, trainable_variables=trainable_variables, learning_rate=self.learning_rate_input_e)
	...
	def build_model(self):
		...
		################### LOSS & OPTIMIZER ##############################################################################################################################
		# Losses.
		self.loss_contrastive, self.logits_ab_contrastive, self.labels_contrastive = self.loss()
		# Optimizers.
		...
		with tf.Session(config=config) as session:
			...
			for epoch in range(1, epochs+1):
				...
				for batch_images, batch_labels in data.training:
					...
					# 	session.run(self.zero_ops,   feed_dict=feed_dict)
